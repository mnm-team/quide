...
namespace Quantum
{
    ...
    ///     <remarks>
    ///         <para>
    ///             QuantumComputer is a singleton, which instance can be obtained by calling
    ///             <see cref="QuantumComputer.GetInstance" /> method.
    ///             QuantumComputer can create and delete the <see cref="Register" /> class (Registers cannot be created
    ///             independently by <b>new</b> operator).
    ///             Quantum computations can be performed by <see cref="Register" /> class, but also by QuantumComputer class,
    ///             by using its extension methods located in namespace <see cref="N:Quantum.Operations" />.
    ///         </para>
    ///     </remarks>
    /// </summary>
    public class QuantumComputer : IQuantumComputer
    {
        ...
        ///     When the absolute amplitude value (real or imaginary) during computations reach the value smaller than Epsilon, it
        ///     is rounded to 0.
        /// </summary>
        ...
        /// </summary>
        /// <param name="initval">
        ///     Unsigned long integer, which binary representation is the initial configuration of qubits in new
        ///     register.
        /// </param>
        ...
        ///     That inner data structure dynamically resizes itself, but if it could be very inefficient, this argument enables to
        ///     reserve enough memory in advance.
        /// </param>
        ...
        {
            if (initStates == null || initStates.Count == 0) return new Register(0, width, _random);
        ...
        ///     Registers consume large amount of memory, so this method is very useful when computations are complex and requires
        ///     many registers.
        /// </summary>
        ...
        ///     This method is used at the begin of every operation from namespace <see cref="N:Quantum.Operations" />, as these
        ///     methods usually operate on more than one register.
        /// </summary>
        /// <param name="refs">
        ///     Registers or references to single qubits. Registers could be passed here, because they are
        ///     implicitly casted to RegisterRef type.
        /// </param>
        ...
        public Register GetRootRegister(params RegisterRef[] refs)
        {
            while (true)
                switch (refs.Length)
                {
                    ...
                    case 2 when refs[0].Register.Root != refs[1].Register.Root:
                    {
                        var newRoot = new Register(_random);
                    ...
                    default:
                    {
                        var firstRoot = GetRootRegister(refs[0], refs[1]);
                        var newParams = new RegisterRef[refs.Length - 1];
        ...
        /// <summary>
        ///     Returns the tensor product of given registers. It has the same result as connecting registers into one in the
        ///     method <see cref="QuantumComputer.GetRootRegister" />.
        ///     However, tensor product could be obtained only when given arguments are independent registers (neither connected
        ///     with nor being a part of any other register).
        /// </summary>
        /// <param name="r1">First register</param>
        /// <param name="r2">Second register</param>
        /// <returns>
        ///     Tensor product, where second register's qubits are least significant bits. Null, if argments are not
        ///     independent registers.
        /// </returns>
        public Register TensorProduct(Register r1, Register r2)
        {
            if (r1 == r1.Root && r2 == r2.Root) return GetRootRegister(r1, r2);
        ...
        {
            return _instance ?? (_instance = new QuantumComputer());
        }
    }
}