...
namespace Quantum.Operations
{
    ...
    ///     </para>
    ///     <para>
    ///         To add two registers, call <see cref="Add(QuantumComputer, Register, Register)" />. To inverse the operation
    ///         (perform a subtraction),
    ///         call <see cref="InverseAdd(QuantumComputer, Register, Register)" />. The rest of methods here are helpers used
    ...
    public static class AddExtension
    {
        ...
        {
            var root = comp.GetRootRegister(rc0, ra0, rb0, rc1);
            var c0 = rc0.OffsetToRoot;
            var a0 = ra0.OffsetToRoot;
            var b0 = rb0.OffsetToRoot;
            var c1 = rc1.OffsetToRoot;
        ...
        ///     Used with <see cref="Carry(QuantumComputer, RegisterRef, RegisterRef, RegisterRef, RegisterRef)" /> to perform
        ///     addition.
        /// </summary>
        /// <param name="comp">The QuantumComputer instance.</param>
        ...
        {
            var root = comp.GetRootRegister(refA, refB, refTarget);
            var a = refA.OffsetToRoot;
            var b = refB.OffsetToRoot;
            var target = refTarget.OffsetToRoot;
            root.CNot(target, a);
            root.CNot(target, b);
        }
            ...
            Register a, Register b, Register c)
        {
            var width = a.Width;
            var i = 0;
            for (; i < width - 1; i++) comp.Carry(c[i], a[i], b[i], c[i + 1]);
        ...
        public static void Add(this QuantumComputer comp, Register a, Register b)
        {
            ...
            var c = comp.NewRegister(0, a.Width);
            comp.Add(a, b, c);
        ...
        /// <param name="b">
        ///     The second register used to inverse the sum. After performing this operation, it contains the inversion
        ///     result.
        /// </param>
        /// <param name="c">The extra register for storing carry bits.</param>
            ...
            Register a, Register b, Register c)
        {
            ...
            i--;
            for (; i >= 0; i--) comp.InverseCarry(c[i], a[i], b[i], c[i + 1]);
        }
        ...
        private static void Validate(Register a, Register b)
        {
            if (b.Width != a.Width + 1)
                throw new ArgumentException("Register b must be exactly one qubit wider than register a.");
        }
    }
}