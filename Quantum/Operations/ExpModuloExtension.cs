...
namespace Quantum.Operations
{
    ...
    public static class ExpModuloExtension
    {
        ...
        ///     </para>
        ///     <para>
        ///         This method is a variant of <c>ExpModulo</c> function, which operates directly on quantum registers given as
        ///         arguments.
        ///         It neither allocates nor frees any additional registers. It is thus recommended, when there is a need for
        ///         performing
        ///         modular exponentiation repeatedly. However, this variant has strict requirements for the width of each given
        ///         register
        ...
        /// <remarks>
        ///     <para>
        ///         There are precise requirements for the width of each register given as argument. They result from a need for
        ///         carry bits,
        ...
        /// <param name="c">
        ///     Register for storing carry bits. Its initial value must be 0. Its width must equal <b>WIDTH + 1</b>
        ///     (See Remarks).
        /// </param>
        ...
        /// <param name="x">
        ///     Register for x. Its initial value could be any integer or a superposition of multiple integers. Its
        ///     width must equal <b>2 * WIDTH</b> (See Remarks).
        /// </param>
        ...
        public static void ExpModulo(
            ...
            int valueN)
        {
            var firstRegisterB = false;
            var pow_a_2 = valueA;

            for (var i = 0; i < x.Width; i++)
            {
                // finding the inversion modulo of pow_a_2
                var inv_mod = InversionModulo(pow_a_2, valueN);
            ...
            int valueN)
        {
            ...
            var a = comp.NewRegister(0, x1.Width - 1);
            var b = comp.NewRegister(0, x1.Width);
            var c = comp.NewRegister(0, x1.Width);
            var N = comp.NewRegister((ulong)valueN, x1.Width - 1);
        ...
        private static void Validate(
            ...
            int valueN)
        {
            if (x.Width != 2 * (x1.Width - 1)) throw new ArgumentException("Width(x) must equal 2 * ( width(x1) - 1 )");
            if (valueN >> (x1.Width - 1) > 0)
                throw new ArgumentException(
                    "Register x1 is too small. It must have enough space to store N, and one qubit more.");
        }
