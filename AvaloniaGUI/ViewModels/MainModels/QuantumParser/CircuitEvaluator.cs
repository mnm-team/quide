...
namespace AvaloniaGUI.ViewModels.MainModels.QuantumParser
{
    public class CircuitEvaluator
    {
        ...
        {
            get { return _comp.GetSourceRoot(); }
        }

        public ComputerModel Model
        {
            get { return _comp.GetModel(); }
        }
        ...
        public OutputViewModel InitFromModel(ComputerModel model)
        {
        ...
        public Register ResolveRegister(string text, bool canBeEmpty)
        {
            ...
            string regex =
                @"^\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*(\[\s*([1-9][0-9]*|[0-9])\s*\,\s*([1-9][0-9]*|[0-9])\s*\])*\s*$";
            Match match = Regex.Match(text, regex);

            if (!match.Success)
            {
                throw new Exception(
                    "Invalid format. Please use the form: <registerName> or <registerName>[<offset>, <width>]");
            }
            ...
            if (match.Groups[2].Success)
            {
                ...
                {
                    throw new Exception(
                        "The offset is not integer. Please use the form: <registerName> or <registerName>[<offset>, <width>]");
                }
                ...
                {
                    throw new Exception(
                        "The width is not integer. Please use the form: <registerName> or <registerName>[<offset>, <width>]");
                }
